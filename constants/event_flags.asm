...
; Johto hidden items
	...
	const EVENT_081		; small open chunk
	const EVENT_082
	const EVENT_083
...
; Crystal-exclusive events in Johto
	...
	const EVENT_BEAT_BREEDER_F_DONNA				; some open chunks
	const EVENT_BEAT_BREEDER_M_CRAIG
	const EVENT_BEAT_RADICAL_TEE
	const EVENT_MEADOW_ITEMBALL1
	const EVENT_MEADOW_ITEMBALL2
	const EVENT_MEADOW_ITEMBALL3
; Kanto story events
...
; ??? NOTE - FREE EVENT SPACES HERE BABY open chunk
	const EVENT_DARKCAVEMIDDLEITEM1
	const EVENT_DARKCAVEMIDDLEITEM2
	const EVENT_DARKCAVEMIDDLEITEM3
	const EVENT_CHERRYGROVE_WATERSTONE
	const EVENT_109
	const EVENT_10A
	const EVENT_10B
	const EVENT_10C
...
; qtdex events HOWEWVER i wasnt very organized yet...
	...
	const EVENT_CIANWOOD_DUNES_MAX_ELIXER
	const EVENT_BEAT_GURU_GOJEE
	const EVENT_BEAT_ENFORCER_WELL
	const EVENT_BEAT_RADICAL_HIDEKI
	const EVENT_BEAT_SAILOR_LAX
	const EVENT_BEAT_SALARYMAN_GORO
	const EVENT_GOT_NOOT_HOUSE_ICE
; Dragon's Depths Events
	const EVENT_BEAT_WARRIOR_MURAI
	const EVENT_DRAGONS_DEPTHS_1F_HIDDEN_FULL_RESTORE
	const EVENT_DRAGONS_DEPTHS_1F_ESCAPE_ROPE
	const EVENT_BEAT_WARRIOR_SHINDO
	const EVENT_BEAT_WARRIOR_MALMESS
	const EVENT_BEAT_WARRIOR_KAZUO
	const EVENT_BEAT_WARRIOR_TETSUO
	const EVENT_BEAT_WARRIOR_GLENN
	const EVENT_DRAGONS_DEPTHS_B1F_STAR_PIECE
	const EVENT_DRAGONS_DEPTHS_B1F_MAX_ELIXER
	const EVENT_DRAGONS_DEPTHS_B1F_WATER_STONE
	const EVENT_DRAGONS_DEPTHS_B1F_ULTRA_BALL
	const EVENT_DRAGONS_DEPTHS_B1F_HIDDEN_MAX_REVIVE
	const EVENT_DRAGONS_DEPTHS_B1F_POTION
	const EVENT_BEAT_GURU_SHOICHI
	const EVENT_DRAGONS_DEPTHS_B2F_HIDDEN_NUGGET
	const EVENT_DRAGONS_DEPTHS_B2F_QUICK_CLAW
	const EVENT_DRAGONS_DEPTHS_B2F_BERSERK_GENE
	const EVENT_BEAT_BURGLAR_TSUCHIYA
	const EVENT_DRAGONS_DEPTHS_ITEM_ROOM_BEAK_FOSSIL
	const EVENT_DRAGONS_DEPTHS_SLAATTEL
	const EVENT_DRAGONS_DEPTHS_B3F_HIDDEN_FIRE_STONE
	const EVENT_DRAGONS_DEPTHS_B3F_HIDDEN_MOON_STONE
	; Extra Mahogony Rocket Trainers
	const EVENT_BEAT_ENGINEER_1
	const EVENT_BEAT_ENGINEER_2
	const EVENT_BEAT_ENGINEER_3
	const EVENT_BEAT_SALARYMAN_CHOTARO
	; Kaiju Gym
	const EVENT_BEAT_TWINS_TO_AND_HO
	const EVENT_BEAT_KAIJU_NOTZILLA
	const EVENT_5F3
	const EVENT_5F4
	const EVENT_5F5
	; Cianwood Dunes Cave Events
	const EVENT_CIANWOOD_DUNES_CAVE_ODD_AMBER
	const EVENT_CIANWOOD_DUNES_CAVE_SUN_STONE
	const EVENT_CIANWOOD_DUNES_CAVE_HIDDEN_MOON_STONE
	const EVENT_BEAT_DELINQUENT_M_CHUSUKE
	const EVENT_BEAT_DELINQUENT_F_CHERRY
	; Cianwood Dunes Events (cont lel)
	const EVENT_BEAT_ATHLETE_F_AVA
	const EVENT_BEAT_SWIMMERM_DALTON
	const EVENT_BEAT_SWIMMERF_WILLOW
	const EVENT_BEAT_FANATIC_R_YAMATO
	const EVENT_BEAT_DELINQUENT_M_TAKEO
	const EVENT_BEAT_FANATIC_B_SAIJO
	const EVENT_CIANWOOD_DUNES_CAVE_FIRE_PUNCH
	const EVENT_BEAT_YOTSUBA
	const EVENT_GOT_TENTAQUIL
	; Andromega events
	const EVENT_GOT_ANDROMEGA
	const EVENT_ANDROMEGA_SUN_STONE
	const EVENT_ANDROMEGA_MOON_STONE
	; rocket computers
	...
	; illuxury event
	const EVENT_CAUGHT_ILLUXURY
	const EVENT_PLACED_BALL_IN_SHRINE
	const ILLUXURY_1_IN_ILEX_FOREST
	const ILLUXURY_2_IN_ILEX_FOREST
	const ILLUXURY_3_IN_ILEX_FOREST
	const ILLUXURY_SPOOKY_MUSIC_STOPS
	const ILLUXURY_TENSION_PLAYING
	const EVENT_BEAT_WIZARD_TIM
	const EVENT_GOT_QT_BALL_FROM_TIM
	; Victory Road
	const EVENT_BOULDER_IN_VICTORY_ROAD_SIDE_WATER
	const EVENT_BEAT_RADICAL_YASUI
	const EVENT_BEAT_SALARYMAN_TURNER
	const EVENT_BEAT_BREEDER_M_BERMAN
	const EVENT_VICTORY_ROAD_THUNDERPUNCH
	const EVENT_VICTORY_ROAD_FULL_RESTORE_2
	const EVENT_VICTORY_ROAD_CILANTRO
	const EVENT_VICTORY_ROAD_FULL_RESTORE_HIDDEN
	const EVENT_VICTORY_ROAD_HIDDEN_NUGGIE
	const EVENT_VICTORY_ROAD_LEAF_STONE_HIDDEN
	const EVENT_BEAT_ATHLETE_SHANNA
	const EVENT_BEAT_FANATIC_AMY
	const EVENT_BEAT_RADICAL_NAOYUKI
	const EVENT_VICTORY_ROAD_REPEL
	const EVENT_VICTORY_ROAD_MAX_REPEL
	; ice path combo breaker
	const EVENT_ICE_PATH_B2F_FOSSIL
	...
	const EVENT_BEAT_HISTORIAN_MCKENNA
	const EVENT_ROUTE41_FOSSIL
	; some trainers
	const EVENT_ROUTE40_ATHLETE_DOC
	const EVENT_ROUTE_45_LEAF_STONE
	const EVENT_ROUTE_45_MAX_REPEL
	const EVENT_BEAT_DELINQUENT_M_GEN
	; trash collection
	const EVENT_TRASH1
	const EVENT_TRASH2
	const EVENT_TRASH3
	const EVENT_TRASH4
	const EVENT_TRASH5
	const EVENT_ALLTRASHGONE
	; artists
	const EVENT_BEAT_BROWN_ARTIST
